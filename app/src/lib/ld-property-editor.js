...
export function initialise_property_editor() {
    LivingDocState.livingdoc.interactiveView.page.focus.componentFocus.add(function (component) {
        component_property_editor(component);
    });
}

export function refresh_property_editor(component) {
    var currentActive = LivingDocState.getActiveComponent();
    if (component && currentActive && currentActive.model.id == component.model.id) {
        component_property_editor(component);
    }
}

function component_property_editor(component) {
    $("." + PROPS_HOLDER).remove();
    $('.' + STYLE_HOLDER).remove();
